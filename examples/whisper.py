...
def beamsearch_sampling(logits: Tensor, model: Whisper, sum_probs: Tensor, ctx: Tensor, _eot: Tensor, beam_size: int=5, topk: int=5):
  eot = enc._special_tokens["<|endoftext|>"]
  log_probs = logits.log_softmax()
  top_probs = collections.defaultdict(list)
  for i in range(beam_size):
    if not _eot[i].tolist():
      for j in range(topk):
        prob = log_probs[i].max(axis=0).tolist()
        idx = log_probs[i].argmax(axis=0).tolist()
        log_probs[i, idx] = -math.inf
        top_probs[i].append((prob, idx))
  candidates = {}
  for i in range(beam_size):
    if not _eot[i].tolist():
      for j in range(topk):
        prob: Tensor = sum_probs[i] + top_probs[i][j][0]
        prob = prob.tolist()
        candidates[prob] = (i, j)
  next_tokens = []
  probs = []
  ...
  finished = None
  finished_prob = None
  for selected in list(sorted(candidates, key=candidates.get, reverse=False)):
    i = saved
    new_i, j = candidates[selected]
    token = top_probs[new_i][j][1]
    if token == eot:
      finished = np.concatenate((ctx[new_i], np.array([token])))
      finished_prob = selected
    else:
      ctx[i] = ctx[new_i]
      indices.append(new_i)
      next_tokens.append(token)
      probs.append(selected)
      saved += 1
    if saved >= 5:
      break
  for block in model.decoder.blocks:
    block.attn.rearrange_kv_cache(indices)
  return Tensor(next_tokens), Tensor(probs), Tensor(ctx), finished, finished_prob

def inferloop(model: Whisper, ctx: np.ndarray, encoded_audio: Tensor, temperature: int, num_sample: int, enc: Encoding) -> np.ndarray:
  eot = enc._special_tokens["<|endoftext|>"]
